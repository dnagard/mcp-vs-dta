...
const JSON_READ_FILE = {
  type: "object",
  properties: {
    path: { type: "string", description: "Relative path under sandbox" },
  },
  required: ["path"],
  additionalProperties: false,
};
...
export const tools = {
  write_file: {
    ...
    handler: async (a: z.infer<typeof WriteFileArgs>) => {
      const p = sp(a.path);
      await writeFileDirect(p, a.data);
  ...
  read_file: {
    ...
    handler: async (a: z.infer<typeof ReadFileArgs>) => {
      const p = sp(a.path);
      const buf = await readFileDirect(p);
  ...
  http_get_json: {
    ...
    handler: async (a: z.infer<typeof GetJsonArgs>) => {
      const json = await httpGetJSON(a.url);
      return { ok: true, url: a.url, json };
    },
  },
  http_get_blob: {
    ...
    handler: async (a: z.infer<typeof GetBlobArgs>) => {
      const ab = await httpGetArrayBuffer(a.url);
      return { ok: true, url: a.url, bytes: ab.byteLength };
    },
  },
} as const;
...
export async function callTool(name: ToolName, rawArgs: any) {
  ...
  if (name === "write_file") {
    if (rawArgs?.filename && !rawArgs?.path) {
      rawArgs.path = rawArgs.filename;
      delete rawArgs.filename;
    }
    if (rawArgs?.content && !rawArgs?.data) {
      rawArgs.data = rawArgs.content;
      delete rawArgs.content;
    }
